	...
	(function() {
		...
		jq('.addEmailContainer').on('click', '.addPhone', function() {
		   ...
		   console.log("Clicked...");
		   jqthis.children('.hidden').eq(0).find('.phoneEl').val("1").end().removeClass('hidden').find('label').html('Email');
	...
	(function() {
		...
		jq('.addRoleContainer').on('click', '.addPhone', function() {
		   ...
		   console.log("hitt mee...");
		   //i = jqthis.children('.hiddenEmailPhone').val(),
		   //index = parseInt(i);
		   //jqthis.children('.row').eq(index).show();
		   jqthis.children('.hidden').eq(0).find('.phoneEl').val("1").end().removeClass('hidden').find('label').html('Role');
		   //jqthis.children('.hiddenEmailPhone').val(++index);
