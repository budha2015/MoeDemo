...
jQuery(document).ready(function() {
   ...
   function adjustmainpanelheight() {
	      ...
	      jQuery('.leftpanel').css('height' , docHeight);
	        mainHeight = jQuery('.mainpanel').height();
 ...
 //View comment Eye click popup 
	

	//open pop up
	
		jQuery(document).on("click", ".viewComment", function(){
		
		var viewCommentsContainer = jQuery(".container #lmsTable_wrapper table tr td .viewCommentsContainer");
		
			if(jQuery(this).parent().children().hasClass("hide")){
			
				viewCommentsContainer.addClass("show");
				viewCommentsContainer.removeClass("hide");		
		...
		//close view comment pop up after clicking outside the pop up
		
		
		jQuery(document).click(function(e){
		
		 var viewCommentsContainer = jQuery(".container #lmsTable_wrapper table tr td .viewCommentsContainer");
		 var eyeImage = jQuery(".container #lmsTable_wrapper table tr td .viewComment");
		 
		 
		 if ((!viewCommentsContainer.is(e.target))&&(!eyeImage.is(e.target))&&(viewCommentsContainer.has(e.target).length === 0)&&(eyeImage.has(e.target).length === 0))
			{
				viewCommentsContainer.addClass("hide");
				viewCommentsContainer.removeClass("show");
				viewCommentsContainer.find('.textArea textarea').val('Start Typing...');
				viewCommentsContainer.find('.comments-reply').css('display','none');
			}
			else{}
		});
		
	
		// for reply button inside the view comment pop up
		
		
		jQuery(document).on('click', '.viewComment-btn-reply', function(e){
		 e.preventDefault();
		 jQuery('.comments-reply').toggle();
		});
